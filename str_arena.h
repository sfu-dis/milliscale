...
struct str_arena {
  ...
  // Return a raw char* starts aligned, with aligned up size
  char *next_raw_aligned(uint64_t alignment, uint64_t size) {
    uint64_t addr = align_up((uint64_t)str + n, (uint64_t)alignment);
    uint64_t off = addr - (uint64_t)str;
    ASSERT(off - n < alignment);
    n += ((off - n) + align_up(size, alignment));
    ASSERT(n < config::arena_size_mb * config::MB);
    return (char *)addr;
  }

  // Same as next but wihtout varstr
  inline char *next_raw(uint64_t size) {
    uint64_t off = n;
    n += align_up(size);
    ASSERT(n < config::arena_size_mb * config::MB);
    return str + off;
