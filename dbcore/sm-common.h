...
struct fat_ptr {
  ...
  static uint64_t const C_SIZE_BITS = 8;
  static uint64_t const SIZE_MASK = (1 << C_SIZE_BITS) - 1;

  static uint64_t const FLAG_START_BIT = C_SIZE_BITS;
  static uint64_t const FLAG_BITS = VALUE_START_BIT - C_SIZE_BITS;
...
/* Transaction ids uniquely identify each transaction in the system
   ...
   the 32-bit sequence number,  a 16-bit local identifier and 16 bits
   for the size and flag. The combination is globally unique, and any
   two transactions that coexist are guaranteed to have different local
   identifiers.

   |------ sequence number (32 bits)------|----id (16 bits) ----|
 */
struct XID {
  static XID make(uint32_t seq, uint16_t id) {
    uint64_t x = seq;
    x <<= 16;
    x |= id;
    x <<= 16;
  ...
  uint16_t id() const { return (_val >> 16) & 0xFFFF; }
  uint32_t seq() const { return _val >> 32; }
...
struct CSN {
  static CSN make(uint64_t c, uint8_t log_id = INVALID_SIZE_CODE) {
    uint64_t x = c;
  ...
  uint64_t _val;
  uint8_t log_id() const { return _val & INVALID_SIZE_CODE; }
