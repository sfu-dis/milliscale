...
static void* calloc_callback(size_t nmemb, size_t size) {
  ...
  char* newMem =
      reinterpret_cast<char*>(Aws::Malloc(MemTag, dataSize + offset));
  std::size_t* pointerToSize = reinterpret_cast<std::size_t*>(newMem);
...
struct CurlWriteCallbackContext {
  CurlWriteCallbackContext(
      const CustomCurlHttpClient* client, CURL* curlHandle,
  ...
  const CustomCurlHttpClient* m_client;
...
static const char* CURL_HTTP_CLIENT_TAG = "CustomCurlHttpClient";
...
static size_t WriteData(char* ptr, size_t size, size_t nmemb, void* userdata) {
  if (ptr) {
    ...
    const CustomCurlHttpClient* client = context->m_client;
    ...
    if (context->m_rateLimiter) {
      context->m_rateLimiter->ApplyAndPayForCost(
          static_cast<int64_t>(sizeToWrite));
    }

    for (const auto& hashIterator :
         context->m_request->GetResponseValidationHashes()) {
      std::stringstream headerStr;
      headerStr << "x-amz-checksum-" << hashIterator.first;
      if (context->m_response->HasHeader(headerStr.str().c_str())) {
        hashIterator.second->Update(reinterpret_cast<unsigned char*>(ptr),
                                    sizeToWrite);
        break;
      }
    }
    ...
    if (response->GetResponseBody().fail()) {
      const auto& ref = response->GetResponseBody();
      AWS_LOGSTREAM_ERROR(CURL_HTTP_CLIENT_TAG,
                          "Unable to query response output position (eof: "
                              << ref.eof() << ", bad: " << ref.bad() << ")");
      return 0;
    }

    response->GetResponseBody().write(
        ptr, static_cast<std::streamsize>(sizeToWrite));
    if (response->GetResponseBody().fail()) {
      const auto& ref = response->GetResponseBody();
      AWS_LOGSTREAM_ERROR(CURL_HTTP_CLIENT_TAG,
                          "Failed to write "
                              << size << " / " << sizeToWrite << " B response"
                              << " at " << cur << " (eof: " << ref.eof()
                              << ", bad: " << ref.bad() << ")");
      return 0;
    }
...
static size_t WriteHeader(char* ptr, size_t size, size_t nmemb,
                          void* userdata) {
  if (ptr) {
    CurlWriteCallbackContext* context =
        reinterpret_cast<CurlWriteCallbackContext*>(userdata);
    ...
    Aws::Vector<Aws::String> keyValuePair =
        StringUtils::Split(headerLine, ':', 2);

    if (keyValuePair.size() == 2) {
      response->AddHeader(StringUtils::Trim(keyValuePair[0].c_str()),
    ...
    // checking for end of all the headers before setting response code
      ...
      long responseCode{-1};
      curl_easy_getinfo(context->m_curlHandle, CURLINFO_RESPONSE_CODE,
                        &responseCode);
...
static size_t ReadBody(char* ptr, size_t size, size_t nmemb, void* userdata,
                       bool isStreaming) {
  CurlReadCallbackContext* context =
      reinterpret_cast<CurlReadCallbackContext*>(userdata);
  if (context == nullptr) {
    return 0;
  }
  ...
  bool isAwsChunked =
      request->HasHeader(Aws::Http::CONTENT_ENCODING_HEADER) &&
      request->GetHeaderValue(Aws::Http::CONTENT_ENCODING_HEADER)
              .find(Aws::Http::AWS_CHUNKED_VALUE) != Aws::String::npos;
  ...
  if (ioStream != nullptr && amountToRead > 0) {
    ...
    if (context->m_rateLimiter) {
      context->m_rateLimiter->ApplyAndPayForCost(
          static_cast<int64_t>(amountRead));
    }
...
static size_t ReadBodyStreaming(char* ptr, size_t size, size_t nmemb,
                                void* userdata) {
  return ReadBody(ptr, size, nmemb, userdata, true);
}

static size_t ReadBodyFunc(char* ptr, size_t size, size_t nmemb,
                           void* userdata) {
...
int CustomCurlHttpClient::CurlProgressCallback(void* userdata, curl_off_t,
                                               curl_off_t, curl_off_t,
                                               curl_off_t)
#else
int CustomCurlHttpClient::CurlProgressCallback(void* userdata, double, double,
                                               double, double)
#endif
{
  CurlReadCallbackContext* context =
  ...
  const std::shared_ptr<Aws::IOStream>& ioStream =
      context->m_request->GetContentBody();
  if (!ioStream || ioStream->bad()) {
    AWS_LOGSTREAM_ERROR(CURL_HTTP_CLIENT_TAG, "Input stream is bad!");
    return 1;  // libcurl abort the transfer and return
  ...
  if (context->m_client->m_perfMode ==
    ...
    // This is a spin pause-unpause in case of no data provided by a customer
    // callback But otherwise curl will slow down the transfer and start calling
    // as at frequency of 1s
    //   see https://curl.se/mail/lib-2020-07/0046.html
    // we should use multi handle or another HTTP client in the future to avoid
    // this
    curl_easy_pause(context->m_curlHandle, CURLPAUSE_CONT);
  } else {
    if (peekVal == std::char_traits<char>::eof()) {
...
std::atomic<bool> CustomCurlHttpClient::isInit(false);

void CustomCurlHttpClient::InitGlobalState() {
  if (!isInit) {
    ...
    AWS_LOGSTREAM_INFO(CURL_HTTP_CLIENT_TAG,
                       "Initializing Curl library with version: "
                           << curlVersionData->version << ", ssl version: "
                           << curlVersionData->ssl_version);
    isInit = true;
#ifdef USE_AWS_MEMORY_MANAGEMENT
    CURLcode curlResponseCode = curl_global_init_mem(
        CURL_GLOBAL_ALL, &malloc_callback, &free_callback, &realloc_callback,
    ...
    if (curlResponseCode != CURLE_OK) {
      AWS_LOGSTREAM_FATAL(
          ...
          "Failed to init curl, return code " << curlResponseCode);
      isInit = false;
    }
  }
}

void CustomCurlHttpClient::CleanupGlobalState() {
  if (isInit) {
    curl_global_cleanup();
    isInit = false;
  }
...
int CurlDebugCallback(CURL* handle, curl_infotype type, char* data, size_t size,
  ...
  if (type == CURLINFO_SSL_DATA_IN || type == CURLINFO_SSL_DATA_OUT) {
    AWS_LOGSTREAM_TRACE(
        "CURL", "(" << CurlInfoTypeToString(type) << ") " << size << " bytes");
  } else if (type == CURLINFO_DATA_IN || type == CURLINFO_DATA_OUT) {
    // for curl data (in/out) traces, print only the data size, as it contains
    // user private data.
    AWS_LOGSTREAM_TRACE(
        "CURL", "(" << CurlInfoTypeToString(type) << ") " << size << " bytes");
  } else {
...
CustomCurlHttpClient::CustomCurlHttpClient(
    ...
    : Base(),
      m_curlHandleContainer(
          ...
          clientConfig.lowSpeedLimit, clientConfig.version),
...
// thread_local CURLM* multi_handle = nullptr;
std::shared_ptr<HttpResponse> CustomCurlHttpClient::MakeRequest(
    const std::shared_ptr<HttpRequest>& request,
    Aws::Utils::RateLimits::RateLimiterInterface* readLimiter,
    Aws::Utils::RateLimits::RateLimiterInterface* writeLimiter) const {
  CURLM* multi_handle;
  bool should_yield;
  bool temp_handle = (CoroTask::current() == nullptr);
  if (temp_handle) {
    // get a temp handler
    ...
    multi_handle = data->multi_handle;
    should_yield = data->should_yield;
  }

  URI uri = request->GetUri();
  Aws::String url = uri.GetURIString();
  std::shared_ptr<HttpResponse> response =
      Aws::MakeShared<StandardHttpResponse>(CURL_HTTP_CLIENT_TAG, request);
  ...
  if (connectionHandle) {
    ...
    CurlWriteCallbackContext writeContext(this, connectionHandle, request.get(),
                                          response.get(), readLimiter);

    const auto readContext = [this, &connectionHandle, &request,
                              &writeLimiter]() -> CurlReadCallbackContext {
      if (request->GetContentBody() != nullptr) {
        auto chunkedBodyPtr = Aws::MakeShared<AwsChunkedStream<>>(
            CURL_HTTP_CLIENT_TAG, request.get(), request->GetContentBody());
        return {this, connectionHandle, request.get(), writeLimiter,
                std::move(chunkedBodyPtr)};
      }
      return {this, connectionHandle, request.get(), writeLimiter};
    }();
    ...
    if (m_verifySSL) {
      ...
      curl_easy_setopt(connectionHandle, CURLOPT_SSLVERSION,
    ...
    if (m_enableHttpClientTrace) {
      ...
      curl_easy_setopt(connectionHandle, CURLOPT_DEBUGFUNCTION,
                       CurlDebugCallback);
    }
    ...
    if (m_isUsingProxy) {
      ...
      if (!m_proxyCaPath.empty()) {
        curl_easy_setopt(connectionHandle, CURLOPT_PROXY_CAPATH,
                         m_proxyCaPath.c_str());
      }
      if (!m_proxyCaFile.empty()) {
        curl_easy_setopt(connectionHandle, CURLOPT_PROXY_CAINFO,
                         m_proxyCaFile.c_str());
      }
#endif
      ...
      curl_easy_setopt(connectionHandle, CURLOPT_NOPROXY,
                       m_nonProxyHosts.c_str());
    ...
    if (request->GetContentBody()) {
      ...
      if (request->IsEventStreamRequest() &&
        ...
        curl_easy_setopt(connectionHandle, CURLOPT_XFERINFOFUNCTION,
                         CustomCurlHttpClient::CurlProgressCallback);
        curl_easy_setopt(connectionHandle, CURLOPT_XFERINFODATA, &readContext);
#else
        curl_easy_setopt(connectionHandle, CURLOPT_PROGRESSFUNCTION,
                         CustomCurlHttpClient::CurlProgressCallback);
    ...
    if (errorBuffer.GetUnderlyingData() &&
        errorBuffer.GetSize() >= CURL_ERROR_SIZE) {
      errorBuffer[0] = '\0';
      curl_easy_setopt(connectionHandle, CURLOPT_ERRORBUFFER,
                       errorBuffer.GetUnderlyingData());
    } else {
      AWS_LOGSTREAM_ERROR(CURL_HTTP_CLIENT_TAG,
                          "Failed to allocate CURLOPT_ERRORBUFFER");
    }

    OverrideOptionsOnConnectionHandle(connectionHandle);
    Aws::Utils::DateTime startTransmissionTime = Aws::Utils::DateTime::Now();

    curl_multi_add_handle(multi_handle, connectionHandle);
    ...
    while (still_running) {
      ...
      if (still_running) {
        if (should_yield) {
          CoroTask::current()->yield();
        } else {
          mc = curl_multi_poll(multi_handle, NULL, 0, 25, NULL);
        }
      }
    }
    ...
    while ((msg = curl_multi_info_read(multi_handle, &msgs_left))) {
      if (msg->msg == CURLMSG_DONE && msg->easy_handle == connectionHandle) {
        curlResponseCode = msg->data.result;
        break;
      }
    }

    curl_multi_remove_handle(multi_handle, connectionHandle);
    if (temp_handle) {
    ...
    if (curlResponseCode != CURLE_OK && shouldContinueRequest) {
      ...
      AWS_LOGSTREAM_ERROR(
          CURL_HTTP_CLIENT_TAG,
          "Curl returned error: " << response->GetClientErrorMessage());
    } else if (!shouldContinueRequest) {
      response->SetClientErrorType(CoreErrors::USER_CANCELLED);
      response->SetClientErrorMessage(
          "Request cancelled by user's continuation handler");
    } else {
      long responseCode;
      curl_easy_getinfo(connectionHandle, CURLINFO_RESPONSE_CODE,
                        &responseCode);
      response->SetResponseCode(static_cast<HttpResponseCode>(responseCode));
      AWS_LOGSTREAM_DEBUG(CURL_HTTP_CLIENT_TAG,
                          "Returned http response code " << responseCode);
      ...
      if (request->GetMethod() != HttpMethod::HTTP_HEAD &&
        ...
        int64_t numBytesResponseReceived =
            writeContext.m_numBytesResponseReceived;
        AWS_LOGSTREAM_TRACE(
            CURL_HTTP_CLIENT_TAG,
    ...
    if (ret == CURLE_OK) {
      request->AddRequestMetric(
          GetHttpClientMetricNameByType(HttpClientMetricsType::ConnectLatency),
          static_cast<int64_t>(timep * 1000));
    }

#if LIBCURL_VERSION_NUM >= 0x073D00  // 7.61.0
    curl_off_t metric;
    ret = curl_easy_getinfo(connectionHandle, CURLINFO_APPCONNECT_TIME_T,
                            &metric);  // Ssl Latency
#else
    double metric;
    ...
    if (ret == CURLE_OK) {
      request->AddRequestMetric(
          GetHttpClientMetricNameByType(HttpClientMetricsType::SslLatency),
          static_cast<int64_t>(metric * 1000));
    }

#if LIBCURL_VERSION_NUM >= 0x073700  // 7.55.0
    ret = curl_easy_getinfo(connectionHandle, CURLINFO_SPEED_DOWNLOAD_T,
                            &metric);  // throughput
#else
    ret = curl_easy_getinfo(connectionHandle, CURLINFO_SPEED_DOWNLOAD,
                            &metric);  // throughput
#endif
    if (ret == CURLE_OK) {
      // Record two metric names to preserve backwards compat
      request->AddRequestMetric(
          GetHttpClientMetricNameByType(HttpClientMetricsType::Throughput),
          static_cast<int64_t>(metric));
      request->AddRequestMetric(
          GetHttpClientMetricNameByType(HttpClientMetricsType::DownloadSpeed),
          static_cast<int64_t>(metric));
    }

#if LIBCURL_VERSION_NUM >= 0x073700  // 7.55.0
    ret = curl_easy_getinfo(connectionHandle, CURLINFO_SPEED_UPLOAD_T,
                            &metric);  // Upload Speed
#else
    ret = curl_easy_getinfo(connectionHandle, CURLINFO_SPEED_UPLOAD,
                            &metric);  // Upload Speed
#endif
    if (ret == CURLE_OK) {
      request->AddRequestMetric(
          GetHttpClientMetricNameByType(HttpClientMetricsType::UploadSpeed),
          static_cast<int64_t>(metric));
    }
    ...
    if (response->GetResponseBody().fail()) {
      const auto& ref = response->GetResponseBody();
      Aws::StringStream ss;
      ss << "Failed to flush response stream (eof: " << ref.eof()
         << ", bad: " << ref.bad() << ")";
    ...
    request->AddRequestMetric(
        GetHttpClientMetricNameByType(HttpClientMetricsType::RequestLatency),
        (DateTime::Now() - startTransmissionTime).count());
  }
