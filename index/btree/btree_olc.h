...
// B+-tree with optimistic lock coupling inherited from OptiQL repo
// (https://github.com/sfu-dis/optiql) using centralized optimistic locks on all nodes.
//
// Supports fixed-length keys only.
...
struct BTreeOLC : public BTreeBase<Key, Value> {
  ...
  BTreeOLC() {
    root = (NodeBase *)ermia::MM::allocate(sizeof(BTreeLeaf<Key, Value>));
    new (root) BTreeLeaf<Key, Value>();
  }
  ...
  restart:
    ...
    uint64_t versionNode = OptLock::kInvalidVersion;
    NodeBase *node = root;
    ...
    while (node->getType() == PageType::BTreeInner) {
      ...
      uint64_t versionNext = OptLock::kInvalidVersion;
  ...
  restart:
    ...
    versionNode = OptLock::kInvalidVersion;
    node = root;
    if (node->getType() == PageType::BTreeLeaf) {
