...
struct NodeBase {
  ...
  // Optimistic lock API
  uint64_t readLockOrRestart(bool &needRestart) const { return lock.TryBeginRead(needRestart); }
  ...
  void readUnlockOrRestart(uint64_t startRead, bool &needRestart) const {
    needRestart = !lock.ValidateRead(startRead);
  }

  void upgradeToWriteLockOrRestart(uint64_t &version, bool &needRestart) {
    // FIXME(shiges): update [version]
    needRestart = !lock.TryLock(version);
  }
...
struct BTreeLeaf : public NodeBase {
  ...
  BTreeLeaf *split(Key &sep) {
    BTreeLeaf *newLeaf = (BTreeLeaf *)ermia::MM::allocate(pageSize);
...
struct BTreeInner : public NodeBase {
  ...
  BTreeInner *split(Key &sep) {
    ...
    BTreeInner *newInner = (BTreeInner *)ermia::MM::allocate(pageSize);
    new (newInner) BTreeInner(level);
...
struct BTreeBase {
  ...
  void makeRoot(Key k, NodeBase *leftChild, NodeBase *rightChild) {
    ...
    BTreeInner<Key> *inner = (BTreeInner<Key> *)ermia::MM::allocate(pageSize);
    new (inner) BTreeInner<Key>(leftChild->level + 1);
    inner->count = 1;
    inner->keys[0] = k;
  ...
  typedef std::function<bool(Key, Value)> Callback;

  // Same as scan but takes start and end keys (exclusive) and a callback function per key met
  uint64_t scan_cb(Key k, Key end_k, Callback cb) {
    int restartCount = 0;
  restart:
    if (restartCount++) yield(restartCount);
    bool needRestart = false;
    ...
    while (leaf) {
      for (unsigned i = pos; i < leaf->count; i++) {
        ASSERT(leaf->data[i].first <= end_k);
        if (leaf->data[i].first == end_k) {
          return count;
        }

        // invoke the callback
        if (!cb(leaf->data[i].first, leaf->data[i].second)) {
          return count;
        }
