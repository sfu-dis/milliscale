...
public:
  ...
  void *allocate(size_t sz, memtag) {
    ...
    new (obj) Object(NULL_PTR, NULL_PTR, true);
...
public:
  ...
  inline bool search(const key_type &k, OID &o,
  ...
  void search_range_call(const key_type &lower, const key_type *upper,
                         ...
                         ermia::oid_array *oa, TXN::xid_context *xc) const;
   ...
   */
  inline bool insert_if_absent(const key_type &k, OID o,
                                        insert_info_t *insert_info = NULL);
...
mbtree<P>::search_range_call(const key_type &lower, const key_type *upper,
  ...
  threadinfo ti;
  table_.scan(lcdf::Str(lower.data(), lower.size()), true, scanner, oa, xc, ti);
}
...
mbtree<P>::rsearch_range_call(const key_type &upper, const key_type *lower,
  ...
  threadinfo ti;
  table_.rscan(lcdf::Str(upper.data(), upper.size()), true, scanner, oa, xc, ti);
}
