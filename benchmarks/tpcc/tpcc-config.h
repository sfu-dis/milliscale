...
DECLARE_uint64(tpcc_payment_remote_wh_pct);
DECLARE_uint64(tpcc_payment_remote_wh_pct);
...
DECLARE_bool(tpcc_log_update_delta);
...
#define DEFN_TBL_INIT_X(name) , tbl_##name##_vec(partitions.at(#name))
 ...
 public:
  tpcc_worker_mixin(const std::map<std::string, std::vector<ermia::UnorderedIndex *>> &partitions)
      : _dummy()  // so hacky...
...
#define DEFN_TBL_ACCESSOR_X(name)                                              \
 private:                                                                      \
  std::vector<ermia::UnorderedIndex *> tbl_##name##_vec;                         \
                                                                               \
 protected:                                                                    \
 ...
 private:
  ...
  static std::vector<ermia::UnorderedIndex *> OpenIndexes(const char *name) {
    ...
    if (FLAGS_tpcc_enable_separate_tree_per_partition && !is_read_only) {
      ...
      ermia::UnorderedIndex *idx = ermia::TableDescriptor::GetIndex(s_name);
      ALWAYS_ASSERT(idx);
      for (size_t i = 0; i < NumWarehouses(); i++) {
        ret[i] = idx;
      }
...
#undef OPEN_TABLESPACE_X
 ...
 private:
  std::map<std::string, std::vector<ermia::UnorderedIndex *>> partitions;
};
