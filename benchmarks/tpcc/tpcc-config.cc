...
DEFINE_uint64(tpcc_payment_remote_wh_pct, 15, "");
...
DEFINE_bool(tpcc_log_update_delta, true, "Whether to log only delta (true) or full record (false) for updates");
...
void tpcc_parse_options() {
  ...
  ALWAYS_ASSERT(FLAGS_tpcc_payment_remote_wh_pct >= 0 &&
                FLAGS_tpcc_payment_remote_wh_pct <= 100);
  ...
  std::cerr << "  log_update_delta             : " << FLAGS_tpcc_log_update_delta << std::endl;
  ...
  std::cerr << "  new_order_remote_item_pct    : " << FLAGS_tpcc_new_order_remote_item_pct
       << std::endl;
  std::cerr << "  payment_remote_wh_pct    : " << FLAGS_tpcc_payment_remote_wh_pct
       << std::endl;
