...
class tpcc_sequential_worker : public bench_worker, public tpcc_worker_mixin {
 public:
  tpcc_sequential_worker(unsigned int worker_id, unsigned long seed, ermia::Engine *db,
              ...
              const std::map<std::string, std::vector<ermia::UnorderedIndex *>> &partitions,
...
rc_t tpcc_sequential_worker::txn_new_order() {
  ...
  if (!FLAGS_tpcc_new_order_fast_id_gen) {
    ...
    v_d_new.d_next_o_id++;
    if (FLAGS_tpcc_log_update_delta) {
      TryCatch(tbl_district(warehouse_id)
                    ->UpdateRecord(txn, Encode(str(Size(k_d)), k_d),
                          Encode(str(Size(v_d_new)), v_d_new),
                          offsetof(district::value, d_next_o_id), (char *)&v_d_new.d_next_o_id, sizeof(v_d_new.d_next_o_id)));
    } else {
      TryCatch(tbl_district(warehouse_id)
                    ->UpdateRecord(txn, Encode(str(Size(k_d)), k_d),
                          Encode(str(Size(v_d_new)), v_d_new)));
...
rc_t tpcc_sequential_worker::txn_payment() {
  ...
  if (likely(FLAGS_tpcc_disable_xpartition_txn || NumWarehouses() == 1 ||
             RandomNumber(r, 1, 100) > FLAGS_tpcc_payment_remote_wh_pct)) {
  ...
  if (FLAGS_tpcc_log_update_delta) {
    TryCatch(tbl_warehouse(warehouse_id)
                  ->UpdateRecord(txn, Encode(str(Size(k_w)), k_w),
                        ...
                        offsetof(warehouse::value, w_ytd), (char *)&v_w_new.w_ytd, sizeof(v_w_new.w_ytd)));
  } else {
    TryCatch(tbl_warehouse(warehouse_id)
                ->UpdateRecord(txn, Encode(str(Size(k_w)), k_w),
                      Encode(str(Size(v_w_new)), v_w_new)));
  ...
  if (FLAGS_tpcc_log_update_delta) {
    TryCatch(tbl_district(warehouse_id)
                  ->UpdateRecord(txn, Encode(str(Size(k_d)), k_d),
                        Encode(str(Size(v_d_new)), v_d_new),
                        offsetof(district::value, d_ytd), (char *)&v_d_new.d_ytd, sizeof(v_d_new.d_ytd)));
  } else {
    TryCatch(tbl_district(warehouse_id)
                  ->UpdateRecord(txn, Encode(str(Size(k_d)), k_d),
                        Encode(str(Size(v_d_new)), v_d_new)));
  ...
  if (new_data) {
    ...
    if (FLAGS_tpcc_log_update_delta) {
      TryCatch(tbl_customer(customerWarehouseID)
                    ->UpdateRecord(txn, Encode(str(Size(k_c)), k_c),
                          Encode(str(Size(v_c_new)), v_c_new),
                          offsetof(customer::value, c_balance), (char *)&v_c_new.c_balance, 
                          sizeof(v_c_new.c_balance) + sizeof(v_c_new.c_ytd_payment) + sizeof(v_c_new.c_payment_cnt)
                          // ^^ taking advantage that these three fields are defined consecutively
                          ));
    } else {
      TryCatch(tbl_customer(customerWarehouseID)
                    ->UpdateRecord(txn, Encode(str(Size(k_c)), k_c),
  ...
  for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      ...
      if (FLAGS_tpcc_log_update_delta) {
        TryCatch(tbl_order_line(warehouse_id)
                      ->UpdateRecord(txn, *c.values[i].first,
                            ...
                            offsetof(order_line::value, ol_delivery_d),
                            (char *)&v_ol_new.ol_delivery_d, sizeof(v_ol_new.ol_delivery_d)));
      } else {
        TryCatch(tbl_order_line(warehouse_id)
                      ->UpdateRecord(txn, *c.values[i].first,
                            Encode(str(Size(v_ol_new)), v_ol_new)));
    ...
    if (FLAGS_tpcc_log_update_delta) {
      TryCatch(tbl_oorder(warehouse_id)
                    ->UpdateRecord(txn, Encode(str(Size(k_oo)), k_oo),
                          ...
                          offsetof(oorder::value, o_carrier_id),
                          (char *)&v_oo_new.o_carrier_id,
                          sizeof(v_oo_new.o_carrier_id)));
    } else {
      TryCatch(tbl_oorder(warehouse_id)
                    ->UpdateRecord(txn, Encode(str(Size(k_oo)), k_oo),
                          Encode(str(Size(v_oo_new)), v_oo_new)));
    ...
    if (FLAGS_tpcc_log_update_delta) {
      TryCatch(tbl_customer(warehouse_id)
                    ->UpdateRecord(txn, Encode(str(Size(k_c)), k_c),
                          Encode(str(Size(v_c_new)), v_c_new),
                          offsetof(customer::value, c_balance),
                          (char *)&v_c_new.c_balance, sizeof(v_c_new.c_balance)));
    } else {
      TryCatch(tbl_customer(warehouse_id)
                    ->UpdateRecord(txn, Encode(str(Size(k_c)), k_c),
                          Encode(str(Size(v_c_new)), v_c_new)));
...
rc_t tpcc_sequential_worker::txn_query2() {
  // TODO(yongjunh): use TXN_FLAG_READ_MOSTLY once SSN's read optimization are available.
  ermia::transaction *txn = db->NewTransaction(0, *arena, txn_buf());
  ermia::scoped_str_arena s_arena(arena);
