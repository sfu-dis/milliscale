...
class bench_loader : public ermia::thread::Runner {
 ...
 protected:
  ...
  std::map<std::string, ermia::UnorderedIndex *> open_tables;
...
class bench_worker : public ermia::thread::Runner {
 public:
  bench_worker(unsigned int worker_id, bool is_worker, unsigned long seed,
               ermia::Engine *db, const std::map<std::string, ermia::UnorderedIndex *> &open_tables,
 ...
 public:
  double total_time = 0;
  double total_on_cpu_time = 0;
  
  // get on cpu time
  double static get_thread_cpu_time() {
      struct timespec ts;
