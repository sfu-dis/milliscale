...
DEFINE_bool(ycsb_log_update_delta, true, "Whether to log only delta (true) or full record (false) for updates");
...
YcsbWorkload YcsbWorkloadU('U', 0, 0, 100U, 0, 0);  // Workload I - 100% update
...
void ycsb_create_db(ermia::Engine *db) {
  ...
  auto create_table = [=](char *) {
    ...
    //db->CreateExHashPrimaryIndex<8>("USERTABLE", std::string("USERTABLE"));
    //db->CreateBTreeOLCPrimaryIndex<8>("USERTABLE", std::string("USERTABLE"));
...
void ycsb_table_loader::do_load(ermia::UnorderedIndex *tbl, std::string table_name, uint64_t hot_record_count, uint64_t cold_record_count) {
  int64_t hot_to_insert = hot_record_count / ermia::config::loaders;
  int64_t cold_to_insert = cold_record_count / ermia::config::loaders;
  ...
  for (uint64_t i = 0; i < hot_to_insert; ++i) {
    ...
    // The value here doesn't carry any meaning, so for simplicity
    // just using the same key field value as "value"
    BuildValue(hot_start_key + i, v);
  ...
  while (loaders_barrier != ermia::config::loaders && !all_flushed);

  if (n == ermia::config::loaders) {
  ...
  for (uint64_t i = 0; i < cold_to_insert; ++i) {
    ...
    ermia::varstr &v = str(0);
    tbl->GetRecord(txn, rc, k, v);
    VerifyValue(cold_start_key + i, v);
    TryVerifyStrict(rc);
...
void ycsb_parse_options() {
  ...
  } else if (FLAGS_ycsb_workload == "U") {
    ycsb_workload = YcsbWorkloadU;
  } else {
  ...
  std::cerr << "ycsb settings:" << std::endl
            ...
            << "  log_update_delta:           " << FLAGS_ycsb_log_update_delta << std::endl;
