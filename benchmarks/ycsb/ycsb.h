...
// 8-byte field size, 10 fields per row (inlcuding one field as the primary key)
#define YCSB_KEY_FIELDS(x, y) x(inline_str_fixed<8>, y_key)
#define YCSB_VALUE_FIELDS(x, y) x(inline_str_fixed<72>, y_value)
...
inline void BuildKey(uint64_t key, ermia::varstr &k) {
  ...
  static const char *prefix = "ycsbdata";  // has to be 8B
...
inline void BuildValue(uint64_t field_value, ermia::varstr &v) {
  ASSERT (sizeof(ycsb_kv::value) % sizeof(uint64_t) == 0);
  ycsb_kv::value extended_value;
  for (uint offset = 0; offset < sizeof(ycsb_kv::value); offset = offset + sizeof(uint64_t)) {
    *(uint64_t *)(extended_value.y_value.data() + offset) = field_value;
  }
  Encode(v, extended_value);
}

inline void VerifyValue(uint64_t field_value, ermia::varstr &v) {
  ycsb_kv::value v_y_temp;
  const ycsb_kv::value *v_y = Decode(v, v_y_temp);
  for (uint offset = 0; offset < sizeof(ycsb_kv::value); offset = offset + sizeof(uint64_t)) {
    ALWAYS_ASSERT(*(uint64_t *)(v_y_temp.y_value.data() + offset) == field_value);
  }
}
...
class ycsb_table_loader : public bench_loader {
 ...
 private:
  uint32_t loader_id;
  void do_load(ermia::UnorderedIndex *tbl, std::string table_name, uint64_t hot_record_count, uint64_t cold_record_count);
...
class ycsb_bench_runner : public bench_runner {
 ...
 protected:
  virtual std::vector<bench_loader *> make_loaders() {
    ...
    uint64_t records_per_thread = std::max<uint64_t>(1, FLAGS_ycsb_hot_table_size / ermia::config::loaders);
    FLAGS_ycsb_hot_table_size = records_per_thread * ermia::config::loaders;
    std::cerr << "[INFO] requested for " << requested << " hot table records, will load "
         << FLAGS_ycsb_hot_table_size << " using " << ermia::config::loaders << " loader threads" << std::endl;

    if (FLAGS_ycsb_cold_table_size > 0) {
      requested = FLAGS_ycsb_cold_table_size;
      records_per_thread = std::max<uint64_t>(1, FLAGS_ycsb_cold_table_size / ermia::config::loaders);
      FLAGS_ycsb_cold_table_size = records_per_thread * ermia::config::loaders;
    ...
    for (uint32_t i = 0; i < ermia::config::loaders; ++i) {
